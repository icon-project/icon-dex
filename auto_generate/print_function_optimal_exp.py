...
print('        uint256 z;')
print('')
print('        z = y = x % 0x{:x}; // get the input modulo 2^({:+d})'.format(hiTerms[0].bit,
                                                                             EXP_MAX_HI_TERM_VAL-EXP_NUM_OF_HI_TERMS))
...
print('        res = res / 0x{:x} + y + FIXED_1; // divide by {}! and then add y^1 / 1! + y^0 / 0!'
      .format(loTerms[0].val, len(loTerms)))
print('')
