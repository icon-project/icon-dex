...
class TestFlexibleToken(IconIntegrateTestBase):
    ...
    def setUp(self, **kwargs):
        ...
        # Deploys flexible_token SCORE
        tx_result = deploy_score(icon_integrate_test_base=super(), content_as_bytes=get_content_as_bytes("flexible_token"),
                                 from_=self._test1, params=deploy_params)

        self.flexible_token_address = tx_result['scoreAddress']
    ...
    def test_flexible_token_property(self):
        actual_token_name = self._icx_call_default(self.flexible_token_address, "name")
        self.assertEqual(self.st_token_name, actual_token_name)

        actual_token_symbol = self._icx_call_default(self.flexible_token_address, "symbol")
        self.assertEqual(self.st_token_symbol, actual_token_symbol)

        actual_total_supply = self._icx_call_default(self.flexible_token_address, "totalSupply")
        self.assertEqual(self._TOKEN_INITIAL_TOTAL_SUPPLY_WITH_DECIMALS, int(actual_total_supply, 0))

        actual_owner_balance = self._icx_call_default(self.flexible_token_address, "balanceOf",
                                                      {"_owner": self._test1.get_address()})
        self.assertEqual(self._TOKEN_INITIAL_TOTAL_SUPPLY_WITH_DECIMALS, int(actual_owner_balance, 0))

        actual_owner = self._icx_call_default(self.flexible_token_address, "getOwner")
        self.assertEqual(self._test1.get_address(), actual_owner)

        actual_new_owner = self._icx_call_default(self.flexible_token_address, "getNewOwner")
        self.assertEqual(str(ZERO_SCORE_ADDRESS), actual_new_owner)

    def test_flexible_token_disable_transfer(self):
        # Success case: transfer_possibility's initial setting should be True
        actual_transfer_possibility = self._icx_call_default(self.flexible_token_address, "getTransferPossibility")
        self.assertTrue(int(actual_transfer_possibility, 0))

        # Failure case: only owner can disable transfer
        send_tx_params = {"_disable": "0x1"}
        self.assertRaises(AssertionError, transaction_call, super(), self._genesis, self.flexible_token_address,
                          "disableTransfer", send_tx_params)

        # Success case: owner can change transfer_possibility True -> False
        send_tx_params = {"_disable": "0x1"}
        transaction_call(super(), self._test1, self.flexible_token_address,
                         "disableTransfer", send_tx_params)
        ...
        transaction_call(super(), self._test1, self.flexible_token_address, "disableTransfer", send_tx_params)
        actual_transfer_possibility = self._icx_call_default(self.flexible_token_address, "getTransferPossibility")
        self.assertTrue(int(actual_transfer_possibility, 0))

    def test_flexible_token_transfer(self):
        token_receiver = create_address()
        transfer_token = 10

        # Success case: transfer 10 flexible token to receiver
        send_tx_params = {"_to": str(token_receiver), "_value": hex(transfer_token)}
        self._transaction_call_default(self.flexible_token_address, "transfer", send_tx_params)

        # Failure case: try to transfer when transfer possibility is False

        # Changes transfer possibility
        send_tx_params = {"_disable": "0x1"}
        self._transaction_call_default(self.flexible_token_address, "disableTransfer", send_tx_params)

        # Transfers 10 flexible token to receiver
        send_tx_params = {"_to": str(token_receiver), "_value": hex(transfer_token)}
        self.assertRaises(AssertionError, self._transaction_call_default, self.flexible_token_address, "transfer",
                          send_tx_params)

    def test_flexible_token_issue(self):
        issue_balance = 10 * 10 ** 18
        # Failure case: only owner can issue the flexible token
        send_tx_params = {"_to": self._fee_treasury.get_address(), "_amount": hex(issue_balance)}
        self.assertRaises(AssertionError, transaction_call, super(), self._genesis, self.flexible_token_address, "issue",
                          send_tx_params)
        ...
        self.assertRaises(AssertionError, self._transaction_call_default, self.flexible_token_address, "issue",
                          send_tx_params)

        # Success case: owner can issue flexible token
        send_tx_params = {"_to": self._fee_treasury.get_address(), "_amount": hex(issue_balance)}
    ...
    def test_flexible_token_destroy(self):
        ...
        self.assertRaises(AssertionError, self._transaction_call_default, self.flexible_token_address,
        ...
        self.assertRaises(AssertionError, transaction_call, super(), self._genesis, self.flexible_token_address, "destroy",
                          send_tx_params)
        ...
        owner_balance_before_destroy = self._icx_call_default(self.flexible_token_address, "balanceOf",
                                                              {"_owner": self._test1.get_address()})
        total_supply_before_destroy = self._icx_call_default(self.flexible_token_address, "totalSupply")

        send_tx_params = {"_from": self._test1.get_address(), "_amount": hex(destroy_balance)}
        self._transaction_call_default(self.flexible_token_address, "destroy", send_tx_params)

        owner_balance_after_destroy = self._icx_call_default(self.flexible_token_address, "balanceOf",
                                                             {"_owner": self._test1.get_address()})
        self.assertEqual(int(owner_balance_before_destroy, 0) - destroy_balance, int(owner_balance_after_destroy, 0))

        total_supply_after_destroy = self._icx_call_default(self.flexible_token_address, "totalSupply")
        ...
        other_balance_before_destroy = self._icx_call_default(self.flexible_token_address, "balanceOf",
                                                              {"_owner": self._fee_treasury.get_address()})
        ...
        other_balance_after_destroy = self._icx_call_default(self.flexible_token_address, "balanceOf",
                                                             {"_owner": self._fee_treasury.get_address()})
        total_supply_after_destroy = self._icx_call_default(self.flexible_token_address, "totalSupply")
    ...
    def test_flexible_token_transfer_ownership(self):
        ...
        send_tx_params = {"_newOwner": new_owner.get_address()}
        self._transaction_call_default(self.flexible_token_address, "transferOwnerShip", send_tx_params)
        ...
        self.assertRaises(AssertionError, self._transaction_call_default, self.flexible_token_address, "acceptOwnerShip",
                          {})
        ...
        tx_result = transaction_call(super(), new_owner, self.flexible_token_address, "acceptOwnerShip", {})
